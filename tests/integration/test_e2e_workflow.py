...
def test_multiple_goals_priority_order(goal_engine):
    ...
    goal_low = goal_engine.create_goal(
        description="Low priority task", mode=GoalMode.GOAL_ORIENTED, priority=2
    )

    goal_medium = goal_engine.create_goal(
        description="Medium priority task", mode=GoalMode.GOAL_ORIENTED, priority=5
    )

    goal_high = goal_engine.create_goal(
        description="High priority task", mode=GoalMode.GOAL_ORIENTED, priority=9
    )
...
def test_goal_error_handling(goal_engine):
    ...
    # Create valid goal
    goal = goal_engine.create_goal(description="Valid goal", mode=GoalMode.GOAL_ORIENTED)
...
def test_goal_status_transitions(goal_engine):
    ...
    goal = goal_engine.create_goal(
        description="Test goal status transitions", mode=GoalMode.GOAL_ORIENTED
    )
...
def test_goal_completion_criteria(goal_engine):
    ...
    criteria = ["All tests pass", "Code review approved", "Documentation updated"]
...
def test_multiple_goal_engines_independent(goal_engine):
    ...
    goal1 = goal_engine.create_goal(description="Goal in engine 1", mode=GoalMode.GOAL_ORIENTED)

    # Create second engine
    engine2 = GoalEngine()

    # Create goal in second engine
    goal2 = engine2.create_goal(description="Goal in engine 2", mode=GoalMode.GOAL_ORIENTED)
