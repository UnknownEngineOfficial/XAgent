...
from xagent.tools.langserve_tools import execute_code, read_file, think, write_file
    ...
    for goal in sub_goals:
        ...
        table.add_row(
            ...
            (
                ...
                else "50%" if goal.status == GoalStatus.IN_PROGRESS else "0%"
            ),
        )
...
async def demo_planners():
    ...
    print_info("Planning Phases: Analyze → Decompose → Prioritize → Validate → Execute")

    table = Table(
        title="LangGraph Planning Output", box=box.ROUNDED, show_header=True, show_lines=True
    )
    ...
    if len(langgraph_plan.actions) > 8:
        table.add_row(
            "...", f"[dim]... and {len(langgraph_plan.actions) - 8} more actions ...[/dim]", "", ""
        )

    console.print(table)

    print_step(4, "Plan Analysis...")
    print_info("Planning Strategy: Multi-stage workflow with validation")
    print_info(
        f"Capabilities Detected: {', '.join(langgraph_plan.metadata.get('capabilities', []))}"
    )
...
"""
    ...
    think_result = await think(
        thought=thought, context={"phase": "analysis", "timestamp": str(datetime.now())}
    )
...
async def demo_security_policies():
    ...
    for scenario in test_scenarios:
        ...
        actual_result = (
            "BLOCKED"
            if result["blocked"]
            else "CONFIRMATION" if result["requires_confirmation"] else "ALLOWED"
        )
        ...
        action_str = (
            str(scenario["action"])[:32] + "..."
            if len(str(scenario["action"])) > 35
            else str(scenario["action"])
        )
    ...
    test_cases = [
        ...
        {
            "context": {"action": "deploy", "env": "production", "status": "approved"},
            "should_block": False,
        },
...
async def demo_metacognition():
    ...
    metrics_table.add_row(
        "Successful Actions", str(metrics["total_actions"] - metrics["errors_detected"])
    )
