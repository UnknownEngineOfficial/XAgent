...
def require_scope(required_scope: str):
    ...
    async def scope_checker(user: User = Depends(verify_token)) -> User:
        ...
        if not user.has_scope(required_scope):
            logger.warning(
                f"User {user.username} lacks required scope: {required_scope}"
            )
            raise HTTPException(
...
def require_any_scope(required_scopes: List[str]):
    ...
    async def scope_checker(user: User = Depends(verify_token)) -> User:
        ...
        if not user.has_any_scope(required_scopes):
            logger.warning(
                f"User {user.username} lacks any required scope: {required_scopes}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
def require_role(required_role: UserRole):
    ...
    async def role_checker(user: User = Depends(verify_token)) -> User:
        ...
        if user.role != required_role:
            logger.warning(
                f"User {user.username} has role {user.role}, required: {required_role}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
